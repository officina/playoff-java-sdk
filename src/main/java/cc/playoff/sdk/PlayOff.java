...
public class PlayOff {
	...
	public static String createJWT(String client_id, String client_secret, String player_id, String[] scopes,
			int expires) {
		...
		String token = signer.sign(claims,
				new JWTSigner.Options().setExpirySeconds(expires).setAlgorithm(Algorithm.HS256));
	...
	public PlayOff(String client_id, String client_secret, PersistAccessToken pac, String domain) {
		this(client_id, client_secret, pac, "v2", domain);
	}
	...
	public void getAccessTokenAsync(final Callback cb) {
		...
		Request request = new Request.Builder().url("https://"+domain+"/auth/token")
				.post(RequestBody.create(MEDIA_TYPE_JSON, json.toString())).build();
			...
			public void onResponse(Response response) throws IOException {
				try {
					...
					Long expires_at = System.currentTimeMillis()
							+ (((Double) token.get("expires_in")).longValue() * 1000);
	 ...
	 * 
	 * @params String method The type of request ['GET', 'POST', 'PUT', 'PATCH',
	 * 'DELETE']
	 * 
	 ...
	 * 
	 * @params Object body The data you would like to send in your POST, PUT,
	 * PATCH requests
	 * 
	 * @params boolean raw Whether you would like the response to be string or a
	 * Map (Useful for images)
	 */
	public Object api(String method, String route, Map<String, String> query, Object body, final boolean raw)
			throws IOException, PlayOffException {
		Builder urlBuilder = new HttpUrl.Builder().scheme("https").host("api."+domain)
		...
		if (method.equalsIgnoreCase("GET")) {
			...
			request = new Request.Builder().url(url).post(RequestBody.create(MEDIA_TYPE_JSON, req_body)).build();
		} else if (method.equalsIgnoreCase("PUT")) {
			request = new Request.Builder().url(url).put(RequestBody.create(MEDIA_TYPE_JSON, req_body)).build();
		} else if (method.equalsIgnoreCase("PATCH")) {
			request = new Request.Builder().url(url).patch(RequestBody.create(MEDIA_TYPE_JSON, req_body)).build();
	...
	public void apiAsync(final String method, final String route, final Map<String, String> query, final Object body,
			final boolean raw, final Callback cb) {
		Map<String, Object> token = null;
		if (pac != null) {
			token = pac.load();
		}
	...
	public void makeRequestAsync(Map<String, Object> token, final String method, String route,
			Map<String, String> query, final Object body, final boolean raw, final Callback cb) {
		final Builder urlBuilder = new HttpUrl.Builder().scheme("https").host("api."+domain)
				.encodedPath("/" + this.version + route);
	...
	public String get_login_url() throws URISyntaxException {
		return "https://"+domain+"/auth?response_type=code&client_id=" + client_id + "&redirect_uri=" + redirect_uri;
	}
